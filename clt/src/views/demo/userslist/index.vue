  ...
  <d2-container>
    ...
    <!-- 编辑 -->
      <el-dialog
          ...
          >
          <div>
           <el-form :model="editform" :rules="Editrules" ref="editform" label-width="80px" class="demo-editform">
...
export default {
  ...
  methods:{
    ...
    async getAllList(){
      ...
      await Axios.post('http://localhost:3000/api/getAllusers',postData).then(res=>{
     ...
     async doDel (row) {
      ...
      await Axios.delete(`http://localhost:3000/api/removeUser/${row.username}`)
      ...
      this.$refs.editform.validate(async (res, wtf) => {
        if (res) {
          ...
          // const res = await this.$api.post('/build/update', postData)
           ...
           await Axios.post(`http://localhost:3000/api/updateUser`,postData).then(res=>{
            console.log(res);
    ...
    async searchRes(searchInfo){
      //  console.log(searchInfo);
        ...
        await Axios.post(`http://localhost:3000/api/searchUser`,postData).then(res=>{
